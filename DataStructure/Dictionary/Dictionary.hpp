...
public:
    virtual ~Dictionary() {}
    virtual bool empty() const=0;   //return true if the dictiionary is empty
    virtual int size() const=0;     //return the number of pairs in the dictionary
    virtual std::pair<const Key, Value>* find(const Key& key)const=0;   //return a pointer of the pair of Key:key
    virtual void delete(const Key& key)=0;   //delete the pair of Key:key
